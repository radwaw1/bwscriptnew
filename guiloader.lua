...
local function createConfigWindow(moduleData)
    ...
    if categoryWindows[name] then
        categoryWindows[name]:Destroy()
        categoryWindows[name] = nil
        return
    end
    ...
    categoryWindows[name] = configFrame
end

local function createCategoryWindow(catName)
    if categoryWindows[catName] then
        categoryWindows[catName]:Destroy()
        categoryWindows[catName] = nil
        return
    end

    local catFrame = Instance.new("Frame")
    catFrame.Size = UDim2.new(0, 280, 0, 420)
    catFrame.Position = UDim2.new(0, 200, 0.5, -210)
    catFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 34)
    catFrame.Parent = screenGui

    local catCorner = Instance.new("UICorner"); catCorner.CornerRadius = UDim.new(0,8); catCorner.Parent = catFrame

    local catTitleBar = Instance.new("Frame")
    catTitleBar.Size = UDim2.new(1,0,0,36)
    catTitleBar.BackgroundColor3 = Color3.fromRGB(20,20,24)
    catTitleBar.Parent = catFrame

    local catTitleLabel = Instance.new("TextLabel")
    catTitleLabel.Size = UDim2.new(1,0,1,0)
    catTitleLabel.BackgroundTransparency = 1
    catTitleLabel.Text = catName
    catTitleLabel.TextColor3 = Color3.fromRGB(255,255,255)
    catTitleLabel.Font = Enum.Font.GothamBold
    catTitleLabel.TextSize = 16
    catTitleLabel.Parent = catTitleBar

    makeDraggable(catFrame, catTitleBar)
    ...
    local url = string.format("https://api.github.com/repos/%s/%s/contents/%s/%s?ref=%s", REPO_USER, REPO_NAME, GITHUB_FOLDER, catName, REPO_BRANCH)
                ...
                if ok2 and res.Success then
                    ...
                    if func then
                        ...
                        if s and mod and mod.Run then
                            ...
                            button.Text = mod.Name or file.name:gsub("%.lua$", "")
    ...
    categoryWindows[catName] = catFrame
end
...
for catName in pairs(categories) do
    ...
    btn.MouseButton1Click:Connect(function()
        createCategoryWindow(catName)
    end)
end

reloadButton.MouseButton1Click:Connect(function()
    for _, win in pairs(categoryWindows) do
        win:Destroy()
    end
    categoryWindows = {}
end)